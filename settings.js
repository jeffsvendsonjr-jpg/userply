(function() {
  ...
  function renderLicenseStatus(status) {
    ...
    document.getElementById('license-status').textContent = normalized.valid ? 'Pro' : 'Free';
  }

  function refreshLicenseStatus() {
    chrome.runtime.sendMessage({ type: 'USERPLY_LICENSE_GET_STATUS' }, function(response) {
      if (chrome.runtime.lastError || !response) return;
      document.getElementById('license-key').value = response.licenseKey || '';
  ...
  document.getElementById('save-license-btn').addEventListener('click', function() {
    var licenseKey = document.getElementById('license-key').value.trim();
    chrome.runtime.sendMessage({ type: 'USERPLY_LICENSE_SAVE', licenseKey: licenseKey }, function(response) {
  ...
  document.getElementById('verify-license-btn').addEventListener('click', function() {
    var licenseKey = document.getElementById('license-key').value.trim();
    chrome.runtime.sendMessage({ type: 'USERPLY_LICENSE_VERIFY', licenseKey: licenseKey }, function(response) {
